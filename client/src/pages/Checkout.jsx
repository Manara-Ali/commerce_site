    ...
    const helperFn = async () => {
      if (status === "success" && !paymentError) {
        const { error } = await stripe.confirmPayment({
          ...
          confirmParams: {
            return_url:
              "https://commerce-site-igmb.onrender.com/payment/success",
          },
        });
  ...
  return (
    <>
      <Helmet>
        ...
        <meta
          name="description"
          content="This page allows you to finalize your purchase and provide payment information."
        />
      ...
      >
        ...
        <form className="border rounded-lg p-3" onSubmit={handleSubmit}>
          ...
          <div className="row">
            {errorMessage && (
              <Alert type="alert-danger" message={errorMessage} />
            )}
          </div>
        </form>
      </div>
